...
class PastEncoderEmbedding(nn.Module):
    ...
    def forward(self, past_cat_en_list, past_num_en_list):
        past_test_embed = self.past_test_embed(past_cat_en_list["past_test"].int())
        past_question_embed = self.past_question_embed(past_cat_en_list["past_question"].int())
        past_tag_embed = self.past_tag_embed(past_cat_en_list["past_tag"].int())
        past_testTag_embed = self.past_testTag_embed(past_cat_en_list["past_testTag"].int())
        ...
        for key in past_num_keys:
            past_num_Linear_list.append(past_num_en_list[key])  # (batch, 20, 1*7)

        past_num_concat = torch.concat(past_num_Linear_list).float()

        num_linear = self.num_linear(past_num_concat).unsqueeze(dim=2)  # 7 -> 32

        past_emb = torch.concat([past_cat_emb, num_linear], dim=-1)  # 64
...
class PastDecoderEmbedding(nn.Module):
    ...
    def forward(self, past_cat_de_list, past_num_en_list):
        past_testTag_embed = self.past_testTag_embed(past_cat_de_list["past_testTag"].int())
        past_interaction_embed = self.past_interaction_embed(past_cat_de_list["past_interaction"].int())
        ...
        past_num_Linear_list = []
        for key in past_num_keys:
            past_num_Linear_list.append(past_num_en_list[key])
        
        past_num_concat = torch.concat(past_num_Linear_list).float()
        num_linear = self.num_linear(past_num_concat).unsqueeze(dim=2)
...
class CurrentEncoderEmbedding(nn.Module):
    ...
    def forward(self, current_cat_en_list, current_num_en_list):
        current_test_embed = self.current_test_embed(current_cat_en_list["current_test"].int())
        current_question_embed = self.current_question_embed(current_cat_en_list["current_question"].int())
        current_tag_embed = self.current_tag_embed(current_cat_en_list["current_tag"].int())
        current_testTag_embed = self.current_testTag_embed(current_cat_en_list["current_testTag"].int())
        ...
        for key in current_num_keys:
            current_num_Linear_list.append(current_num_en_list[key])
        
        current_num_concat = torch.concat(current_num_Linear_list).float()
        num_linear = self.num_linear(current_num_concat).unsqueeze(dim=2)
...
class CurrentDecoderEmbedding(nn.Module):
    ...
    def forward(self, current_cat_de_list, current_num_de_list):
        current_testTag_embed = self.current_testTag_embed(current_cat_de_list["current_testTag"].int())
        current_interaction_embed = self.current_interaction_embed(current_cat_de_list["current_interaction"].int())
        ...
        current_num_Linear_list = []
        for key in current_num_keys:
            current_num_Linear_list.append(current_num_de_list[key])
...
class SAINTRec(nn.Module):
    def __init__(self,
        ...
        self.past_decoder_blocks = SAINTBlock_Decoder(n_heads, hd, drop_out)
        ...
        self.current_decoder_blocks = SAINTBlock_Decoder(n_heads, hd, drop_out)
        # LSTM layer
    ...
    def forward(self, past_encoder_feat, past_decoder_feat, current_encoder_feat, current_decoder_feat):
        ...
        # past_embedding
        past_encoder_embedding = PastEncoderEmbedding(past_encoder_feat["past_cat_en_list"], past_encoder_feat["past_num_en_list"]).to('cuda')
        past_en_emb = past_encoder_embedding(past_encoder_feat["past_cat_en_list"], past_encoder_feat["past_num_en_list"]) 
        
        past_encoder_embedding = PastDecoderEmbedding(past_decoder_feat["past_cat_de_list"],past_decoder_feat["past_num_de_list"]).to('cuda')
        past_de_emb = past_encoder_embedding(past_decoder_feat["past_cat_de_list"],past_decoder_feat["past_num_de_list"])
        ...
        # Encoder
        for block in self.past_encoder_blocks.to('cuda'):
            past_en_emb, attn_dist = block(past_en_emb, mask)

        # lstm
        past_en_emb, _ = self.past_lstm(past_en_emb)

        # Decoder
        past_emb, _ = self.past_decoder_blocks(past_de_emb, past_en_emb)
        # for block in self.past_decoder_blocks:
        #     past_emb, attn_dist = block(past_de_emb, past_en_emb, mask)


        # current_embedding
        current_encoder_embedding = PastEncoderEmbedding(current_encoder_feat["past_cat_en_list"], current_encoder_feat["past_num_en_list"]).to('cuda')
        current_en_emb = current_encoder_embedding(current_encoder_feat["past_cat_en_list"], current_encoder_feat["past_num_en_list"]) 

        current_decoder_embedding = PastDecoderEmbedding(current_decoder_feat["past_cat_de_list"], current_decoder_feat["past_num_de_list"]).to('cuda')
        current_de_emb = current_decoder_embedding(current_decoder_feat["past_cat_de_list"], current_decoder_feat["past_num_de_list"])


        # Encoder
        for block in self.current_encoder_blocks.to('cuda'):
            current_en_emb, attn_dist = block(current_en_emb, mask)

        # lstm
        current_en_emb, _ = self.current_lstm(current_en_emb)

        # Decoder
        current_emb, _ = self.current_decoder_blocks(current_de_emb, current_en_emb)
