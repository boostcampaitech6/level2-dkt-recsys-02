...
class SignedConv(MessagePassing):
    ...
    def forward(
        ...
        if isinstance(x, Tensor):
            x = (x, x) #edge니까 Node 두개를 Pair 하게 붙여서 사용해야 한다.
        # propagate_type: (x: PairTensor)
        if self.first_aggr:
            out_pos = self.propagate(pos_edge_index, x=x)
            out_pos = self.lin_pos_l(out_pos)
            out_pos = out_pos + self.lin_pos_r(x[1])#보니까 우측인것 같은데?(from-to)
            ...
            out_pos1 = self.propagate(pos_edge_index, #input feature의 뒷부분 절삭해버리네? 필요없다 이건가 ..
                                      x=(x[0][..., :F_in], x[1][..., :F_in]))
    ...
    def message(self, x_j: Tensor) -> Tensor: ##x_i = from, x_j = to node
        return x_j
