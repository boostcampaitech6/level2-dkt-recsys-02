...
class DKTDataset_PC(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        # Load from data
        test, question, tag, correct, Dffclt, Dscrmn, Gussng = row[0], row[1], row[2], row[3], row[4], row[5], row[6]
        #, testTag, user_correct_answer, user_total_answer, user_acc, user_mean, relative_answer_mean, time_to_solve, time_to_solve_mean, prior_testTag_frequency = row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12], row[13], row[14], row[15]
