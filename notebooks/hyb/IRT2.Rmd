...
```{r}
replace_missing_values <- function(df, col) {
  replacement_values <- sample(c(1, 0), size = sum(df[[col]] == -1), replace = TRUE, prob = c(0.7, 0.3))
  df[[col]][df[[col]] == -1] <- replacement_values
  return(df)
}

test <- replace_missing_values(test, "answerCode")
```
...
# 유저 학습수준 dataframe 만들기
```{r, message=FALSE}
final_levels <- data.frame()
...
for (testId in unique_testids){
  ...
  scores <- sc$score.dat$z1
  userID <- row.names(table_matrix)

  user_levels <- data.frame(
    userID = userID,
    testId = testId,
    user_level = scores
  )

  final_levels <- rbind(final_levels, user_levels)
}
```
...
for (testId in unique_testids){
  ...
  scores <- tryCatch({
    ...
    message("Error in factor.scores: ", e)
    error_test <<- c(error_test, testId)
    rep(0, length(row.names(table_matrix)))
  })
...
```{r}
final_levels$userID <- as.integer(final_levels$userID)
```

```{r}
write.csv(final_levels, "IRT_user_level.csv", row.names = FALSE)
```

```{r}
# train과 test에 left join
train_data <- read.csv("train_data_3PL.csv")
test_data <- read.csv("test_data_3PL.csv")
```


```{r}
joined_train <- train_data %>%
  left_join(final_levels, by = c("userID", "testId"))
joined_test <- test_data %>%
  left_join(final_levels,  by = c("userID", "testId"))
```
...
```{r}
write.csv(joined_train, "train_data_3PL+level.csv", row.names = FALSE)
write.csv(joined_test, "test_data_3PL+level.csv", row.names = FALSE)
```
